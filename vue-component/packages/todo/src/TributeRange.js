...
class TributeRange {
  ...
  pasteHtml(html, startPos, endPos, customSelection) {
    let range, sel;
    sel = this.getWindowSelection();
    range = this.getDocument().createRange();
    range.setStart(
      customSelection ? this.tribute.anchorNode.text : sel.anchorNode,
      startPos
    );
    range.setEnd(
      customSelection ? this.tribute.anchorNode.text : sel.anchorNode,
  ...
  getContentEditableSelectedPath(ctx, customSelection) {
    let sel = this.getWindowSelection();
    let selected = customSelection
      ? this.tribute.anchorNode.text
      : sel.anchorNode;
    let path = [];
    let offset;
    ...
    if (selected != null) {
      ...
      offset = customSelection
        ? this.tribute.anchorNode.offset
  ...
  getTextPrecedingCurrentSelection(customSelection) {
    let context = this.tribute.current,
      text = "";
    if (!this.isContentEditable(context.element)) {
      ...
      let selectedElem = customSelection
        ? this.tribute.anchorNode.text
        : this.getWindowSelection().anchorNode;
      let selectStartOffset = customSelection
        ? this.tribute.anchorNode.offset
        : this.getWindowSelection().getRangeAt(0).startOffset;
      // if (selectedElem != this.tribute.current.element) {
      //   text = "//";
      // } else
